...
if (!empty($_POST)) {
    ...
    if(empty($errors)){
        if($names !== '' && $tiku !== ''){
            $answers = $db_attend->prepare('INSERT INTO answers
                                            (
                                                user_id,
                                                schedule_id,
                                                area,
        ...
        $answer_id = $db_attend->lastInsertId();
        if($answer_id != 0){
            $d = 0;
            foreach ($date as $day) {
                if($date[0] !== ''){
                    $answer_days = $db_attend->prepare('INSERT INTO answer_days
                                                        (
                                                            schedule_id,
                                                            answer_id,
                                                            answer_date,
            ...
            <div class="form_area content">
                <div class="form_content">
                    <div class="form_flex_item">
                        ...
                        <input type="text" name="tiku" id="tiku" value="<?php echo $is_post ? $tiku: ''?>">
                    </div>
                    <div class="form_flex_item">
                        <h3><label for="names">名前</label></h3>
                        <input type="text" id="names" name="names" value="<?php echo $is_post ? $names: ''?>">
                    </div>
                    <div class="form_flex_item" id="answer_select">
                        <h3><p>出欠</p></h3>
                        <?php 
                        ...
                        <?php foreach($schedules as $schedule): $sn++?>
                            ...
                            <select name="syukketu<?php echo $sn ?>" id="syukketu<?php echo $sn ?>" onchange="syukketuNo('meal<?php echo $sn ?>')">
                                <option value="0" <?php echo $is_post ? ($syukketu[$sn-1] == 0 ? 'selected' : '') : ''?>>欠席</option>
                                <option value="1" <?php echo $is_post ? ($syukketu[$sn-1] == 1 ? 'selected' : '') : 'selected'?>>出席</option>
                            </select>
                        <?php endforeach; ?>
                    </div>
                    <div class="form_flex_item">
                            ...
                            <div class="meal_item">
                                ...
                                <div class="form_flex_item" id="meal<?php echo $s ?>">
                                    <?php if($schedule['attendance_asa']): ?>
                                        <label for="asa<?php echo $s ?>">朝<input type="checkbox" value="1" name="asa<?php echo $s ?>" id="asa<?php echo $s ?>" <?php echo $is_post ? !empty($asa[$s-1]) ? 'checked': '': ''?>><span></span></label>
                                    <?php endif; ?>
                                    <?php if($schedule['attendance_hiru']): ?>
                                        <label for="hiru<?php echo $s ?>">昼<input type="checkbox" value="1" name="hiru<?php echo $s ?>" id="hiru<?php echo $s ?>" <?php echo $is_post ? !empty($hiru[$s-1]) ? 'checked': '': ''?>><span></span></label>
                                    <?php endif; ?>
                                        <?php if($schedule['attendance_yuu']): ?>
                                        <label for="yuu<?php echo $s ?>">夕<input type="checkbox" value="1" name="yuu<?php echo $s ?>" id="yuu<?php echo $s ?>" <?php echo $is_post ? !empty($yuu[$s-1]) ? 'checked': '': ''?>><span></span></label>
                                    <?php endif; ?>
                                    <?php if($schedule['attendance_tomari']): ?>
                                        <label for="haku<?php echo $s ?>">泊<input type="checkbox" value="1" name="haku<?php echo $s ?>" id="haku<?php echo $s ?>" <?php echo $is_post ? !empty($haku[$s-1]) ? 'checked': '': ''?>><span></span></label>
                                    <?php endif; ?>
                                </div>
                            </div>
                        <?php endforeach; ?>
